...
struct VarShapeOSC : Module {
    ...
    float voicing[10][4] = { {1.f, getVoicingRatio(7), getVoicingRatio(2), getVoicingRatio(0)},
                            {1.f, getVoicingRatio(7), getVoicingRatio(2), getVoicingRatio(-8)},
                            {1.f, getVoicingRatio(9), getVoicingRatio(4), getVoicingRatio(-7)},
                            {1.f, getVoicingRatio(9), getVoicingRatio(5), getVoicingRatio(2)},
                            ...
                            {1.f, getVoicingRatio(7), getVoicingRatio(1), getVoicingRatio(-3)},
	...
	VarShapeOSC() {
		...
		configParam(VOICING_PARAM, 0.f, 9.f, 0.f, "voicing selection");
	...
	void process(const ProcessArgs& args) override {
        if (outputs[OUTSQR_OUTPUT].isConnected() || outputs[OUTSAW_OUTPUT].isConnected()) {
            ...
            voicing_select = inputs[VOICINGCV_INPUT].isConnected() ? ((int)abs(inputs[VOICINGCV_INPUT].getVoltage()) % (int)paramQuantities[VOICING_PARAM]->getMaxValue()) : (int)(params[VOICING_PARAM].getValue());
            for(int i = 0; i < 4; i++) {
